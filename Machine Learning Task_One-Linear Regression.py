...
plt.plot(X_test['Area'], y_predicted, color='red', linewidth=1, label='Regression Line')
'''
...
slope = model.coef_[0]  # Assuming 'Area' is the first feature
intercept = model.intercept_

# Generate x values for the trendline
x_values = np.linspace(X_test['Area'].min(), X_test['Area'].max(), 100)

# Calculate corresponding y values for the trendline
y_values = slope * x_values + intercept

# Plot the Linear Regression Trendline
plt.plot(x_values, y_values, color='green', linestyle='-', label='Linear Regression Trendline')
'''
